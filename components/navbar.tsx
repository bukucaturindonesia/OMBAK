...
export function Navbar() {
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 border-b border-white/10 bg-ocean-950/82 text-white shadow-[0_10px_40px_rgba(6,21,31,0.18)] backdrop-blur-xl">
      <nav
          ...
          <div className="mx-auto flex max-w-7xl flex-col gap-3">
            {navItems.map((item) => (
              <a
                ...
                className="rounded-2xl px-3 py-3 text-sm font-medium text-white/80 transition hover:bg-white/8 hover:text-gold-300"
              >
                {item.label}
              </a>
            ))}
